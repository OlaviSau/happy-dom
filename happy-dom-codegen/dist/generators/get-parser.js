...
export function generateGetParser(ir) {
	let out = fileHeader();
...
\t */
\tprivate static collapseGlobal(values: string[], important: boolean): ICSSStyleDeclarationPropertyValue | null {
\t\tif (values.length === 0) {
\t\t\treturn null;
\t\t}
...
\t\t\t\treturn null;
\t\t\t}
\t\t\tif (widths[i]!.important !== imp || styles[i]!.important !== imp || colors[i]!.important !== imp) {
\t\t\t\treturn null;
\t\t\t}
\t\t}
...
\t\tprops: PropertyStore,
\t\twidthProp: string, styleProp: string, colorProp: string
\t): ICSSStyleDeclarationPropertyValue | null {
...
\t\t\treturn null;
\t\t}
\t\tif (tl.important !== tr.important || tr.important !== br.important || br.important !== bl.important) {
\t\t\treturn null;
\t\t}
...
\t\tconst globalCollapse = this.collapseGlobal([grow.value, shrink.value, basis.value], grow.important);
\t\tif (globalCollapse) {
\t\t\treturn globalCollapse;
\t\t}
...
\t\t\treturn null;
\t\t}
\t\tconst globalCollapse = this.collapseGlobal([type.value, position.value, image.value], type.important);
\t\tif (globalCollapse) {
\t\t\treturn globalCollapse;
\t\t}
...
\t\t\treturn null;
\t\t}
\t\tconst globalCollapse = this.collapseGlobal([line.value, style.value, color.value], line.important);
\t\tif (globalCollapse) {
\t\t\treturn globalCollapse;
\t\t}
...
\t\tif (!image || !color || !repeat || !posX || !posY || !size ||
...
\t\t// Check global keyword collapse
\t\tconst allValues = [image, color, repeat, posX, posY, size, attachment, origin, clip].map(v => v!.value);
\t\tconst globalCollapse = this.collapseGlobal(allValues, imp);
\t\tif (globalCollapse) {
\t\t\treturn globalCollapse;
\t\t}
...
\t\tif (image.value === 'initial' && repeat.value === 'initial' &&
\t\t\tposX.value === 'initial' && posY.value === 'initial' &&
\t\t\tsize.value === 'initial' && attachment.value === 'initial' &&
\t\t\torigin.value === 'initial' && clip.value === 'initial' &&
\t\t\tcolor.value !== 'initial') {
...
\t\t\treturn null;
\t\t}
\t\tif (source.important !== slice.important || slice.important !== width.important ||
\t\t\twidth.important !== outset.important || outset.important !== repeat.important) {
\t\t\treturn null;
\t\t}
...
\t\t\tif (width.value !== 'initial' && width.value !== '1' || outset.value !== 'initial' && outset.value !== '0') {
...
\t\t} else if (width.value !== 'initial' && width.value !== '1' || outset.value !== 'initial' && outset.value !== '0') {
...
\t *
	 ...
	 * @returns Composed value or null.
\t */
\tprivate static getGeneric(name: string, props: PropertyStore): ICSSStyleDeclarationPropertyValue | null {
...
// # sourceMappingURL=get-parser.js.map
