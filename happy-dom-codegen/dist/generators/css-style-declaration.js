 ...
 * @param camel
 */
function upperVendorVariant(camel) {
 ...
 * @param ir
 */
export function generateCSSPropertyNameMap(ir) {
	let out = fileHeader();
...
`;
	...
	// 'float' is a reserved word so it needs a quoted key
 ...
 * @param ir
 */
export function generateCSSDeclarations(ir) {
	const accessorProps = [];
	const seen = new Set();
	for (const prop of ir.properties) {
		if (prop.name.startsWith('-internal-')) {
