...
function pickValidValue(prop) {
    ...
    if (prop.keywords.length > 0) {
        ...
        const nonInitial = prop.keywords.find((k) => k !== 'initial' && k !== 'inherit' && k !== 'unset' && k !== 'revert');
...
function pickSecondValue(prop) {
    ...
    if (prop.keywords.length > 1) {
        ...
        return (prop.keywords.find((k) => k !== first && k !== 'initial' && k !== 'inherit' && k !== 'unset' && k !== 'revert') ?? 'inherit');
    }
    return 'inherit';
}
/**
 ...
 * @param prop
 */
function pickShorthandValue(prop) {
...
export function generatePropertyTests(ir) {
    const lines = [];
    let testCount = 0;
    ...
    lines.push(` */`);
    lines.push(``);
    lines.push(`import CSSStyleDeclaration from '../out-compiled/css/declaration/CSSStyleDeclaration.js';`);
    lines.push(``);
    ...
    lines.push(`  } else {`);
    lines.push(`    failed++;`);
    lines.push(`    failures.push(\`\${message}: expected \${JSON.stringify(expected)}, got \${JSON.stringify(actual)}\`);`);
    lines.push(`  }`);
    lines.push(`}`);
    lines.push(``);
    ...
    for (const prop of ir.longhands) {
        ...
        lines.push(`  assertEq(s.${camel}, '${esc(val1)}', '${prop.name}: set/get round-trip via accessor');`);
        testCount++;
        // T2: getPropertyValue matches accessor
        lines.push(`  assertEq(s.getPropertyValue('${prop.name}'), '${esc(val1)}', '${prop.name}: getPropertyValue matches accessor');`);
        testCount++;
        // T3: overwrite with second value
        lines.push(`  s.${camel} = '${esc(val2)}';`);
        lines.push(`  assertEq(s.${camel}, '${esc(val2)}', '${prop.name}: overwrite with second value');`);
        testCount++;
        // T4: removeProperty
        ...
        lines.push(`  assertEq(s.getPropertyValue('${prop.name}'), 'inherit', '${prop.name}: accepts inherit');`);
        testCount++;
        ...
        if (kwOnly) {
            ...
            lines.push(`  assertEq(s.${camel}, '${esc(val1)}', '${prop.name}: rejects invalid keyword');`);
            testCount++;
        }
        lines.push(`}`);
        lines.push(``);
    ...
    for (const prop of ir.shorthands) {
        ...
        for (const [lh, expected] of Object.entries(sv.expected)) {
            lines.push(`  assertEq(s.getPropertyValue('${lh}'), '${esc(expected)}', '${prop.name}: expands to ${lh}');`);
            testCount++;
        }
        // T2: global keyword 'inherit' on shorthand expands to all longhands
        lines.push(`  s.setProperty('${prop.name}', 'inherit');`);
        for (const lh of prop.longhands) {
            lines.push(`  assertEq(s.getPropertyValue('${lh}'), 'inherit', '${prop.name}: inherit expands to ${lh}');`);
            testCount++;
        }
        // T3: removeProperty removes all longhands
        lines.push(`  s.removeProperty('${prop.name}');`);
        for (const lh of prop.longhands) {
            lines.push(`  assertEq(s.getPropertyValue('${lh}'), '', '${prop.name}: removeProperty clears ${lh}');`);
            testCount++;
        }
        lines.push(`}`);
        lines.push(``);
    ...
    for (const prop of ir.aliases) {
        ...
        if (canonical.type === 'longhand') {
            lines.push(`  assertEq(s.getPropertyValue('${prop.aliasFor}'), '${esc(testVal)}', '${prop.name}: alias sets canonical ${prop.aliasFor}');`);
            testCount++;
        }
        else {
            // Shorthand: check that at least one longhand got set
            if (canonical.longhands.length > 0) {
                lines.push(`  assertEq(s.getPropertyValue('${canonical.longhands[0]}'), '${esc(testVal)}', '${prop.name}: alias expands via ${prop.aliasFor} to ${canonical.longhands[0]}');`);
                testCount++;
            }
        }
        ...
        if (canonical.type === 'longhand') {
            lines.push(`  assertEq(s2.getPropertyValue('${prop.name}'), '${esc(testVal)}', '${prop.name}: reading alias returns canonical value');`);
            testCount++;
        }
        else {
            ...
            lines.push(`  assertEq(s2.getPropertyValue('${prop.name}').includes('${esc(testVal)}'), true, '${prop.name}: reading alias returns composed value');`);
            testCount++;
        }
        lines.push(`}`);
        lines.push(``);
    ...
    lines.push(`console.log(\`Per-property tests: \${passed} passed, \${failed} failed, \${passed + failed} total\`);`);
