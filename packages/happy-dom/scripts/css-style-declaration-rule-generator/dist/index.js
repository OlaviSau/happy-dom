...
import { generatePropertyDefinitions, generateShorthandDefinitions, generateAliasDefinitions } from './generators/property-definitions.js';
...
import { generateCSSDeclarations, generateCSSPropertyNameMap } from './generators/css-style-declaration.js';
const ROOT = path.resolve(import.meta.dirname ?? path.dirname(new URL(import.meta.url).pathname), '..');
const CSS_PROPERTIES_URL = 'https://chromium.googlesource.com/chromium/src/+/main/third_party/blink/renderer/core/css/css_properties.json5?format=TEXT';
/**
 ...
 * @param destPath
 */
async function downloadCSSProperties(destPath) {
    ...
    console.log(`[codegen] Saved to ${path.relative(ROOT, destPath)} (${(content.length / 1024).toFixed(0)} KB)`);
}
async function main() {
    ...
    if (!offline) {
        ...
        if (!fs.existsSync(jsonPath)) {
            console.error('[codegen] --offline specified but no cached file at data/css-properties.json5');
            process.exit(1);
        }
    ...
    console.log(`[codegen] IR: ${ir.properties.length} properties (${ir.longhands.length} longhands, ${ir.shorthands.length} shorthands, ${ir.aliases.length} aliases)`);
    ...
    const declSourcePath = path.resolve(ROOT, '..', '..', 'src', 'css', 'declaration', 'CSSStyleDeclaration.ts');
    ...
    if (startIdx === -1 || endIdx === -1) {
        throw new Error('Could not find BEGIN_CSS_DECLARATIONS / END_CSS_DECLARATIONS markers in CSSStyleDeclaration.ts');
    }
    const declarations = generateCSSDeclarations(ir);
    const spliced = declSource.slice(0, startIdx + START_MARKER.length) + declarations + declSource.slice(endIdx);
    files.push({
