 ...
 *
 * The CSS property type declarations (between BEGIN_CSS_DECLARATIONS and
 * END_CSS_DECLARATIONS markers) are auto-generated by happy-dom-codegen.
 * All other code is manually maintained.
 */
...
// Reverse map: kebab → canonical camelCase (first-seen wins, so cssFloat beats float)
const CSS_KEBAB_TO_CAMEL = new Map<string, string>(
	Object.entries(CSSStyleDeclarationPropertyNameMap).map(
		([camel, kebab]) => [kebab, camel] as const
	)
);
...
export default class CSSStyleDeclaration {
	...
	static #proxyHandler: ProxyHandler<CSSStyleDeclaration> = {
		get(target: CSSStyleDeclaration, prop: string | symbol, _receiver: unknown): unknown {
			...
			return CSS_KEBAB_PROPERTIES.has(prop)
		...
		ownKeys(target: CSSStyleDeclaration): ArrayLike<string | symbol> {
			...
			for (const camel of Object.keys(CSSStyleDeclarationPropertyNameMap)) {
				keys.add(camel);
			}
